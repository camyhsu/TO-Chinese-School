...
def family_already_processed(person_id, processed_family_id)
  family = DEV_DB["SELECT id FROM families WHERE parent_one_id = ? OR parent_two_id = ?", person_id, person_id].first
  if family.nil?
    family = DEV_DB["SELECT family_id as id FROM families_children WHERE child_id = ?", person_id].first
  end
