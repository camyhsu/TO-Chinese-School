...
describe SchoolYear do
  ...
  it 'should validate that start date is not later then end date' do
    @school_year.start_date = PacificDate.today
    @school_year.end_date = PacificDate.today
    @school_year.valid?.should be_true

    @school_year.start_date = PacificDate.tomorrow
...
describe SchoolYear, 'finding current and future school years' do
  it 'should find current and future years' do
    ...
    SchoolYear.expects(:all).with(:conditions => ["end_date >= ?", PacificDate.today], :order => 'start_date ASC').once.returns(fake_school_years)
...
def create_fake_school_year
  ...
  school_year.registration_start_date = PacificDate.today
  school_year.pre_registration_end_date = PacificDate.today
  school_year.registration_75_percent_date = PacificDate.today
  school_year.registration_50_percent_date = PacificDate.today
  school_year.registration_end_date = PacificDate.today
  school_year.refund_75_percent_date = PacificDate.today
  school_year.refund_50_percent_date = PacificDate.today
  school_year.refund_25_percent_date = PacificDate.today
  school_year.refund_end_date = PacificDate.today
  school_year
end

def setup_current_school_year
  @current_school_year = create_fake_school_year
  @current_school_year.start_date = PacificDate.today
  @current_school_year.end_date = PacificDate.today
...
def setup_one_school_years_starting_in_near_future
  ...
  @first_school_year_starting_in_near_future.start_date = PacificDate.today + 1
  @first_school_year_starting_in_near_future.end_date = PacificDate.today + 10
...
def setup_two_school_years_starting_in_near_future
  ...
  @second_school_year_starting_in_near_future.start_date = PacificDate.today + 2
  @second_school_year_starting_in_near_future.end_date = PacificDate.today + 10
